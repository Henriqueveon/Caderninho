...
> **Design system (Seção 8):** "Minimalismo sofisticado" rose gold, temável por tokens. Fonte única da verdade em `src/styles/tokens.css` (cor primária `#B76E79` + gradiente; superfícies creme/nude; sombras quentes; raios 18–32px; Plus Jakarta Sans). Tailwind mapeia as classes semânticas (`bg-background`, `text-primary`, etc.) para os tokens — trocar o tema = mudar só o bloco de cor primária. Modo claro/escuro com identidade própria (toggle na sidebar, persistido). Marca recriada em SVG (`src/components/brand/Logo.tsx`). Verificado por screenshots (login, dashboard, agenda, financeiro; claro e escuro).
